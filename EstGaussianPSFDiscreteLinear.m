function [SigEst, result] = EstGaussianPSFDiscrete(I1, I2, params)
% created on Jul 9, 2015
% Estimate Gaussian PSF fit using discrete search
...
CostSmoothingWindow = [];
CostValidWindowMask = ones(size(I1, 1), size(I1, 2));
SigmaC = 0;
...
if(isfield(params, 'CostValidWindowMask'))
    CostValidWindowMask = params.CostValidWindowMask;
end
...
for sigRIdx = 1:nDivs
    ...
    cost2D = (Ib - Iblurred).^params.expK; % TODO: use an evaluator function NCC, SSD, SAD, etc
...
SigEst = mean(mean(result.SigEst(CostValidWindowMask)));